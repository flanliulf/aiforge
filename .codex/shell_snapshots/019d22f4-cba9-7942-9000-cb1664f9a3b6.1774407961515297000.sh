...
_async_worker () {
	...
	close_idle_coproc () {
		local -a pids
		pids=(${${(v)jobstates##*:*:}%\=*}) 
		if (( ! processing )) && [[ $#pids = 1 ]] && [[ $coproc_pid = $pids[1] ]]
		then
			coproc :
			coproc_pid=0 
		fi
...
_git-authors () {
	_arguments -C '(--list -l)'{--list,-l}'[show authors]' '--no-email[without email]'
}
_git-brv () {
	_arguments '(-r --reverse)'{-r,--reverse}'[reverse order]'
}
_git-buildpackage () {
	# undefined
	builtin autoload -XUz
...
_git-changelog () {
	_arguments '(-l --list)'{-l,--list}'[list commits]'
}
_git-clear () {
	_arguments '(-f --force)'{-f,--force}'[force clear]' '(-h --help)'{-h,--help}'[help message]'
}
_git-coauthor () {
	_arguments ':co-author[co-author to add]:__gitex_author_names' ':co-author-email[email address of co-author to add]:__gitex_author_emails'
}
_git-contrib () {
	_arguments ':author:__gitex_author_names'
}
_git-count () {
	_arguments '--all[detailed commit count]'
}
_git-create-branch () {
	local curcontext=$curcontext state line 
	_arguments -C ': :->command' '*:: :->option-or-argument'
	case "$state" in
		(command) _arguments '(--remote -r)'{--remote,-r}'[setup remote tracking branch]' ;;
...
_git-delete-branch () {
	_arguments ':branch-name:__gitex_branch_names'
}
_git-delete-squashed-branches () {
	_arguments ':branch-name:__gitex_branch_names'
}
_git-delete-submodule () {
	_arguments ':submodule-name:__gitex_submodule_names'
}
_git-delete-tag () {
	_arguments ':tag-name:__gitex_tag_names'
}
_git-effort () {
	_arguments '--above[ignore file with less than x commits]'
}
_git-extras () {
	...
	case $state in
		(command) declare -a commands
			commands=('update:update git-extras') 
			_describe -t commands command commands && ret=0  ;;
	esac
	_arguments '(-v --version)'{-v,--version}'[show current version]'
}
_git-feature () {
	local curcontext=$curcontext state line ret=1 
	declare -A opt_args
	...
	case $state in
		(command) declare -a commands
			commands=('finish:merge feature into the current branch') 
			_describe -t commands command commands && ret=0  ;;
		(option-or-argument) curcontext=${curcontext%:*}-$line[1]: 
			case $line[1] in
				(finish) _arguments -C '--squash[Use squash merge]' ':branch-name:__gitex_feature_branch_names' ;;
				(-r | --remote) _arguments -C ':remote-name:__gitex_remote_names' ;;
			esac
			return 0 ;;
	esac
	_arguments '(--remote -r)'{--remote,-r}'[setup remote tracking branch]'
}
_git-graft () {
	_arguments ':src-branch-name:__gitex_branch_names' ':dest-branch-name:__gitex_branch_names'
}
_git-guilt () {
	_arguments -C '(--email -e)'{--email,-e}'[display author emails instead of names]' '(--ignore-whitespace -w)'{--ignore-whitespace,-w}'[ignore whitespace only changes]' '(--debug -d)'{--debug,-d}'[output debug information]' '-h[output usage information]'
}
_git-ignore () {
	_arguments -C '(--local -l)'{--local,-l}'[show local gitignore]' '(--global -g)'{--global,-g}'[show global gitignore]' '(--private -p)'{--private,-p}'[show repo gitignore]' '*:filename:_files'
}
_git-info () {
	_arguments -C '(--color -c)'{--color,-c}'[use color for information titles]' '--no-config[do not show list all variables set in config file, along with their values]'
}
_git-merge-into () {
	_arguments '--ff-only[merge only fast-forward]'
	_arguments ':src:__gitex_branch_names' ':dest:__gitex_branch_names'
}
_git-missing () {
	_arguments ':first-branch-name:__gitex_branch_names' ':second-branch-name:__gitex_branch_names'
}
_git-release () {
	_arguments -C '-c[Generates/populates the changelog with all commit message since the last tag.]' '-r[The "remote" repository that is destination of a push operation.]' '-m[use the custom commit information instead of the default message.]' '-s[Create a signed and annotated tag.]' '-u[Create a tag, annotated and signed with the given key.]' '--semver[If the latest tag in your repo matches the semver format requirement, you could increase part of it as the new release tag.]' '--prefix[Add a prefix string to semver to allow more complex tags.]' '--no-empty-commit[Avoid creating empty commit if nothing could be committed.]' '--[The arguments listed after "--" separator will be passed to pre/post-release hook.]'
}
_git-rename-file () {
	_arguments -C '-h[show usage information]' '--help[show usage information]' '1:source:__git_files' '2:destination:__git_files'
}
_git-squash () {
...
_git-undo () {
	_arguments -C '(--soft -s)'{--soft,-s}'[only rolls back the commit but changes remain un-staged]' '(--hard -h)'{--hard,-h}'[wipes your commit(s)]'
}
_git_log_prettily () {
	if ! [ -z $1 ]
	then
...
_spaceship () {
	local -a cmds
	cmds=('bug-report:Create a GitHub issue with information about your environment' 'add:Add a section to the prompt at specific position' 'edit:Edit Spaceship config' 'remove:Remove a section from prompt' 'print:Print Spaceship prompt' 'version:Print Spaceship version' 'help:Print this help message') 
	if (( CURRENT == 2 ))
	then
		_describe 'command' cmds
	elif (( CURRENT > 2 ))
	then
		local -a sections
		for section in ${(kM)functions:#spaceship_*}
		do
			sections+=("${section##spaceship_}") 
		done
		case "${words[2]}" in
			(add) local -a subcmds=("${sections[@]}") 
				...
				_describe 'command' subcmds ;;
			(remove) local -a subcmds=("${sections[@]}") 
				subcmds+=("--order: An order to include the section to") 
				subcmds+=("-O: An order to include the section to") 
				_describe 'command' subcmds ;;
			(print) local -a subcmds=() 
...
_spaceship::cli::add () {
	zparseopts -E -D - A:=after_ -after:=after_ B:=before_ -before:=before_ O:=order_ -order:=order_
	...
	then
		...
		new_order=("${(@)order[0,$((index-1))]}" "${sections[@]}" "${(@)order[${index},$]}") 
	fi
	if [[ -n "$after_section" ]]
	then
		index="${order[(i)${after_section}]=0}" 
		new_order=("${(@)order[0,${index}]}" "${sections[@]}" "${(@)order[$((index+1)),$]}") 
	fi
...
_spaceship::cli::bug-report () {
	...
	local os
	case "$kernel_name" in
		("Linux" | "GNU"* | *"BSD") if [[ -f "/etc/os-release" || -f "/usr/lib/os-release" || -f "/etc/openwrt_release" ]]
			then
			...
			os="${os/NAME=}"  ;;
		("Darwin") os="$(sw_vers -productName) $(sw_vers -productVersion)"  ;;
		("CYGWIN"* | "MSYS"* | "MINGW"*) os="$(wmic os get Caption)" 
			os="${os/Caption}" 
			os="${os/Microsoft }"  ;;
	esac
	...
	declare -A environment=([spaceship-version]="$spaceship_version" [zsh-version]="$zsh_version" [os-system]="$os" [zsh-framework]="$frameworks" [terminal]="$terminal") 
	local issue_params="" 
	for key value in "${(@kv)environment}"
	do
	...
	if spaceship::exists xdg-open
	then
		xdg-open "$url"
	elif spaceship::exists open
	then
		open "$url"
	elif spaceship::exists python
	then
...
_spaceship::cli::urlencore () {
	if spaceship::exists python3
	then
		python3 -c 'import urllib.parse, sys; print(urllib.parse.quote(sys.argv[1]))' "$1"
	elif spaceship::exists python2
	then
...
_zsh_highlight_highlighter_main_paint () {
	...
	precommand_options=('-' '' 'builtin' '' 'command' :pvV 'exec' a:cl 'noglob' '' 'doas' aCu:Lns 'nice' n: 'pkexec' '' 'sudo' Cgprtu:AEHPSbilns:eKkVv 'stdbuf' ioe: 'eatmydata' '' 'catchsegv' '' 'nohup' '' 'setsid' :wc 'env' u:i 'ionice' cn:t:pPu 'strace' IbeaosXPpEuOS:ACdfhikqrtTvVxyDc 'proxychains' q:f 'ssh-agent' aEPt:csDd:k 'tabbed' gnprtTuU:cdfhs 'chronic' :ev 'ifne' :n) 
...
claudecodeui () {
	npm start --prefix /Users/fancyliu/ai_sourcecode_space/ide/claudecodeui "$@"
}
clipcopy () {
...
spaceship_package::maven () {
	...
	local maven_exe=$(spaceship::upsearch mvnw)  || (
		spaceship::exists mvn && maven_exe="mvn" 
	) || return
...
alias 9='cd -9'
alias _='sudo '
alias aiideupdate='~/update_ai_tools.sh'
alias codex='CODEX_HOME="$PWD/.codex" codex'
alias copilotapi='npx copilot-api@latest start --proxy-env'
alias copilotx='https_proxy=$PROXY_HTTP http_proxy=$PROXY_HTTP all_proxy=$PROXY_SOCKS copilot --yolo'
...
alias gac='https_proxy= http_proxy= all_proxy= HTTPS_PROXY= HTTP_PROXY= ALL_PROXY= npx -y https://gaccode.com/claudecode/install'
alias gacd='https_proxy= http_proxy= all_proxy= HTTPS_PROXY= HTTP_PROXY= ALL_PROXY= npx -y https://gaccode.com/claudecode/install --dangerously-skip-permissions'
alias gacdr='https_proxy= http_proxy= all_proxy= HTTPS_PROXY= HTTP_PROXY= ALL_PROXY= npx -y https://gaccode.com/claudecode/install --dangerously-skip-permissions -r'
alias gacr='https_proxy= http_proxy= all_proxy= HTTPS_PROXY= HTTP_PROXY= ALL_PROXY= npx -y https://gaccode.com/claudecode/install -r'
alias gam='git am'
...
alias jdk17='export JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk-17.0.5.jdk/Contents/Home'
...
# exports 86
export BUNDLED_DEBUGPY_PATH=/Users/fancyliu/.cursor/extensions/ms-python.debugpy-2025.18.0-darwin-arm64/bundled/libs/debugpy
export BUN_INSTALL=/Users/fancyliu/.bun
export CALIBRE_HOME=/Applications/calibre.app/Contents/MacOS
export CLASSPATH=/lib/ext/ApacheJMeter_core.jar:/lib/jorphan.jar:/Library/Java/JavaVirtualMachines/adoptopenjdk-8.jdk/Contents/Home/lib/tools.jar:/Library/Java/JavaVirtualMachines/adoptopenjdk-8.jdk/Contents/Home/lib/dt.jar:.
export CLAUDE_CODE_SSE_PORT=25511
export CODEX_HOME=/Users/fancyliu/IdeaProjects/wshoto-gitlab-workspace/ai-forge/.codex
export CODEX_MANAGED_BY_NPM=1
export COLORTERM=truecolor
export COMMAND_MODE=unix2003
export CONDA_DEFAULT_ENV=base
export CONDA_EXE=/Users/fancyliu/anaconda3/bin/conda
export CONDA_PREFIX=/Users/fancyliu/anaconda3
export CONDA_PROMPT_MODIFIER='(base) '
export CONDA_PYTHON_EXE=/Users/fancyliu/anaconda3/bin/python
export CONDA_SHLVL=1
export CURSOR_TRACE_ID=60094a17c7f0455699bbfacf919cd209
export FIG_NEW_SESSION=1
export GEMINI_CLI_IDE_AUTH_TOKEN=ac87677f-c8fc-4784-9ba1-df717f3a2243
export GEMINI_CLI_IDE_SERVER_PORT=50310
export GEMINI_CLI_IDE_WORKSPACE_PATH=/Users/fancyliu/IdeaProjects/wshoto-gitlab-workspace/ai-forge
export GIT_ASKPASS=/Applications/Cursor.app/Contents/Resources/app/extensions/git/dist/askpass.sh
export GK_GL_ADDR=http://127.0.0.1:50309
export GK_GL_PATH=/var/folders/m8/1dsqp1x11bq5mwvk4tjdf0cw0000gn/T/gitkraken/gitlens/gitlens-ipc-server-25221-50309.json
export GOOGLE_CLOUD_PROJECT=durable-cacao-476111-m9
export GO_HOME=/usr/local/go
export HOME=/Users/fancyliu
...
export INFOPATH=/usr/local/share/info:/usr/local/share/info:/usr/local/share/info:
export JAVA_17_HOME=/Library/Java/JavaVirtualMachines/jdk-17.0.5.jdk/Contents/Home
export JAVA_20_HOME=/Library/Java/JavaVirtualMachines/zulu-20.jdk/Contents/Home
export JAVA_8_HOME=/Library/Java/JavaVirtualMachines/adoptopenjdk-8.jdk/Contents/Home
export JAVA_8_ZULU_HOME=/Library/Java/JavaVirtualMachines/zulu-8.jdk/Contents/Home
export JAVA_HOME=/Library/Java/JavaVirtualMachines/adoptopenjdk-8.jdk/Contents/Home
export LANG=C.UTF-8
export LESS=-R
export LOGNAME=fancyliu
export LSCOLORS=Gxfxcxdxbxegedabagacad
export LS_COLORS='di=1;36:ln=35:so=32:pi=33:ex=31:bd=34;46:cd=34;43:su=30;41:sg=30;46:tw=30;42:ow=30;43'
export M2_HOME=/usr/local/apache-maven-3.8.4
export MYSQL_HOME=/usr/local/mysql
export MallocNanoZone=0
export NVM_BIN=/Users/fancyliu/.nvm/versions/node/v22.16.0/bin
export NVM_CD_FLAGS=-q
export NVM_DIR=/Users/fancyliu/.nvm
export NVM_INC=/Users/fancyliu/.nvm/versions/node/v22.16.0/include/node
export OSLogRateLimit=64
export PAGER=less
export PSQL_HOME=/Applications/Postgres.app/Contents/Versions/14
export PYDEVD_DISABLE_FILE_VALIDATION=1
export SHELL=/bin/zsh
export SPACESHIP_CONFIG=''
export SPACESHIP_ROOT=/Users/fancyliu/.oh-my-zsh/custom/themes/spaceship-prompt
export SPACESHIP_VERSION=4.15.2
export SSH_AUTH_SOCK=/private/tmp/com.apple.launchd.Fypx3ZIPta/Listeners
export STARSHIP_SESSION_KEY=2635208152174320
export STARSHIP_SHELL=zsh
export TERM=xterm-256color
export TERM_PROGRAM=vscode
export TERM_PROGRAM_VERSION=2.6.14
export TMPDIR=/var/folders/m8/1dsqp1x11bq5mwvk4tjdf0cw0000gn/T/
export USER=fancyliu
export USER_ZDOTDIR=/Users/fancyliu
export VSCODE_DEBUGPY_ADAPTER_ENDPOINTS=/Users/fancyliu/.cursor/extensions/ms-python.debugpy-2025.18.0-darwin-arm64/.noConfigDebugAdapterEndpoints/endpoint-550432d4daf8e088.txt
...
export VSCODE_GIT_IPC_AUTH_TOKEN=72d816367bdf9d4ff998247c8a2c16ae9da244f5af322992cf93bb69a2beea59
export VSCODE_GIT_IPC_HANDLE=/var/folders/m8/1dsqp1x11bq5mwvk4tjdf0cw0000gn/T/vscode-git-60e16ee781.sock
export VSCODE_INJECTION=1
export VSCODE_PROFILE_INITIALIZED=1
export XPC_FLAGS=0x0
export XPC_SERVICE_NAME=0
export ZDOTDIR=/Users/fancyliu
export ZSH=/Users/fancyliu/.oh-my-zsh
export _CE_CONDA=''
export _CE_M=''
