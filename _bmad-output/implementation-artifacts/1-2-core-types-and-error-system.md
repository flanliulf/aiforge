...
- [ ] Task 1: 创建 `src/core/types.ts` — 管道数据契约 (AC: #1)
  - [ ] 1.1 定义 `ParsedArgs` 接口（CLI 参数解析结果）
  - [ ] 1.2 定义 `ResolvedSource` 接口（知识源解析结果：hostname, repoPath, protocol）
  - [ ] 1.3 定义 `AuthenticatedSource` 接口（认证后的源：含 authMethod, cloneUrl）
  - [ ] 1.4 定义 `LocalRepo` 接口（本地仓库：repoDir, isNew, sourceFiles[]）
  - [ ] 1.5 定义 `DetectedEnv` 接口（检测到的工具环境：tools[], scope）
  - [ ] 1.6 定义 `MatchedPlan` 接口（匹配的安装计划：items[]，每项含 rule, sourceFiles, targetPath）
  - [ ] 1.7 定义 `InstallResult` 接口（安装结果：items[]，每项含 status, sourcePath, targetPath）
  - [ ] 1.8 定义 `InstallRule` 接口（安装规则：tool, scope, sourceDir, type, targetDir）
  - [ ] 1.9 定义 `ToolDefinition` 接口（工具定义：id, name, detect.global[], detect.project[]）
  - [ ] 1.10 定义 `ManifestEntry` 接口（manifest 条目：source, target, tool, scope, mode, hash, installedAt）
  - [ ] 1.11 定义 `AiforgeConfig` 接口（配置：defaultRepo, cloneDir, language, preferSSH, auth）
  - [ ] 1.12 定义 `Severity` 类型：`'fatal' | 'partial'`
  - [ ] 1.13 定义 `InstallType` 枚举：`Files`, `Directories`, `Flatten`（PascalCase 值）
  - [ ] 1.14 定义 `AuthMethod` 类型：`'ssh' | 'token' | 'credential-manager'`
- [ ] Task 2: 创建 `src/core/errors.ts` — 统一错误类型 (AC: #2)
  - [ ] 2.1 实现 `AiforgeError extends Error`，含 `code`, `exitCode`, `severity`, `why`, `fix[]`
  - [ ] 2.2 定义退出码常量：`EXIT_SUCCESS = 0`, `EXIT_INSTALL_FAILURE = 1`, `EXIT_AUTH_FAILURE = 2`, `EXIT_ARG_ERROR = 3`
  - [ ] 2.3 提供工厂函数简化常见错误创建（如 `authError()`, `argError()`）
- [ ] Task 3: 创建 `src/core/sanitize.ts` — Token 脱敏 (AC: #3, #4)
  - [ ] 3.1 实现 `sanitizeToken(token: string): string` — 前 8 + `****` + 后 4
  - [ ] 3.2 实现 `sanitizeUrl(url: string): string` — 检测 URL 中嵌入的 Token 并脱敏
- [ ] Task 4: 编写单元测试 (AC: #1-5)
  - [ ] 4.1 `tests/core/errors.test.ts` — AiforgeError 构造、属性、工厂函数
  - [ ] 4.2 `tests/core/sanitize.test.ts` — sanitizeToken 各种长度、sanitizeUrl 各种格式
- [ ] Task 5: 验证模块边界 (AC: #5)
  - [ ] 5.1 确认 core/ 下所有文件无 `import from '../stages'` 等外部依赖
  - [ ] 5.2 确认所有 import 带 `.js` 扩展名
  - [ ] 5.3 确认只使用命名导出
