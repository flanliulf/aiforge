...
- [x] [AI-Review][LOW] 补充 AC#1 缺失断言：目录结构存在性（`src/core/`、`tests/core/` 等子目录）、`tsconfig.json` 关键字段（`strict: true`、`module: NodeNext`、`moduleResolution: NodeNext`）；补充 AC#3 的 `passWithNoTests` 场景验证 [tests/core/toolchain.test.ts]
- [x] [AI-Review][LOW] 消除 `src/index.ts` 中 CLI 版本号硬编码，改为从 `package.json` 动态读取 [src/index.ts]
- [x] [AI-Review][MEDIUM] 移除 `src/index.ts` 中 `createRequire(...)/require('../package.json')` 用法，遵守 Story ESM 约束”不使用 require()” [src/index.ts:3]
- [x] [AI-Review][LOW] 补充 `--version` 行为验证，证明动态版本读取与 `package.json` 一致 [src/index.ts:10]
- [x] [AI-Review][LOW] 同步更新 `_bmad-output/planning-artifacts/architecture/04-implementation-patterns.md`，与 `project-context.md` 中”工具配置文件默认导出豁免”保持一致 — **已完成**
...
- ✅ 已解决 review 问题 [MEDIUM]：移除 `createRequire/require('../package.json')`，改用纯 ESM 方式（`readFileSync` + `fileURLToPath(import.meta.url)`）
- ✅ 已解决 review 问题 [LOW]：补充目录结构存在性断言（10 个子目录）、tsconfig 关键字段断言（strict/module/moduleResolution）、passWithNoTests 场景验证
- ✅ 已解决 review 问题 [LOW]：补充 `--version` 行为验证（`it.skipIf` 在 dist 存在时运行，输出与 package.json version 一致）
- 全部 25 个测试通过，构建成功，lint 无报错
