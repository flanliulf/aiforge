...
3. **Given** `data/messages.ts` **When** 检查实现 **Then** 所有用户可见字符串通过 messages 模块获取，支持根据 `language` 配置切换中英文
...
- [ ] Task 1: 扩展 `data/messages.ts` — 添加英文文案 (AC: #3)
  ...
  - [ ] 1.5 添加所有错误消息英文版
  - [ ] 1.6 导出 `getMessage(key: string, language: string): string` 函数
...
- [ ] Task 3: 实现语言加载和回退逻辑 (AC: #2, #4, #5)
  - [ ] 3.1 管道启动时从 `config.json` 读取 `language`
  - [ ] 3.2 无配置或非法值 → 回退到 `zh-CN`，输出提示
  - [ ] 3.3 将 language 传递给 Reporter 和 messages 模块
...
function resolveLanguage(configLang?: string): Language {
  ...
  if (configLang) {
    // 非法值，输出提示
    console.error(`⚠️ 不支持的语言 "${configLang}"，使用默认中文`);
  }
  return 'zh-CN';
}
```
