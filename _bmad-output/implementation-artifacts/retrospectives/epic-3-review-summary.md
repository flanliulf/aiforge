...
基于上述审查结论，已在 commit `26bb786` 中完成以下修正：

### Story 3.1: AI 工具自动检测

**已修正**：

1. 检测语义统一为"同时扫描全局 + 项目两侧，任一侧命中即视为已安装" — AC #1、Task 1.3~1.6 全部重写
2. 明确检测使用 `pathResolver.home()` + `detect.global[]` 和 `process.cwd()` + `detect.project[]`，不使用 `pathResolver.toolGlobalDir()`（后者是 aiforge 自身的安装目标目录，与真实工具目录无关）
3. Task 1.6 补充说明"scope 决定安装范围，不决定检测范围——检测始终扫描两侧"
4. Dev Notes 的 `detectSingleTool()` 示例代码完全重写，移除 scope 参数，改为同时检查 globalBase 和 projectBase
5. 模块边界说明更新为"使用 `PathResolver.home()` 获取全局检测基准路径（不使用 `toolGlobalDir()`）"

### Story 3.2: 规则匹配引擎

**已修正**：

1. AC #2 补充 MVP 临时约束说明：`--dirs` 当前直接匹配 `rule.sourceDir` 字面值，是对规则数据的过滤而非对仓库结构的假设；后续扩展应在规则层进行
2. 目标路径解析收口回 PathResolver：`resolveTargetDir()` 示例重写，全局路径使用 `pathResolver.home()` + 相对路径，不再手工展开 `~/`
3. Dev Notes 补充说明 `process.cwd()` 用于项目级路径是 MVP 简化处理，后续应替换为 `pathResolver.projectRoot()`

### Story 3.3: dry-run 预览与安装计划输出

**已修正**：

1. Task 1.1 补充说明 `reportPlan()` 在架构文档中的缺失情况，明确以 Story 1.3 为准
2. AC #5 改写为明确的验收边界："Detect → Match 使用真实实现，Resolve → Auth → Clone 的真实接入由 Epic 2 完成后自然生效，本 Story 验收不要求前序阶段为真实实现"
3. 安装模式确定逻辑修正：`scope === 'project' && args.link` 必须抛出 `AiforgeError(code: 'LINK_PROJECT_REJECTED', severity: 'fatal')`，不再静默降级为 copy — 与 FR-021 和 project-context.md 一致
4. `getInstallMode()` 示例代码补充项目级 `-l` 的显式拒绝逻辑
5. 前序阶段占位说明改为"保持占位不影响验收——验收重点是 Detect + Match + Reporter 分叉路径的正确性"

### 架构文档同步

`architecture/03-core-decisions.md` 的 Reporter 接口代码块补齐 `reportPlan(plan: MatchedPlan): void`（commit `26bb786`），后续在 commit `d4ca0b1` 中进一步补齐 `warn(message: string): void` 及 stdout/stderr 分工表。

### 修正覆盖评估

| 审查问题 | 修正状态 |
|---------|---------|
| Story 3.1 工具检测路径契约冲突 | ✅ 已修正（统一为 `home()` + 标志路径，与 `toolGlobalDir()` 解耦） |
| Story 3.2 `--dirs` 过度绑定物理目录名 | ✅ 已修正（标注为 MVP 临时约束，明确扩展路径） |
| Story 3.2 目标路径解析职责漂移 | ✅ 已修正（收口回 PathResolver） |
| Story 3.3 `reportPlan()` 契约未在架构文档收口 | ✅ 已修正（架构文档补齐） |
| Story 3.3 `-l` 处理与 FR-021 冲突 | ✅ 已修正（项目级 `-l` 显式拒绝） |
| 检测语义（按 scope 单侧 vs 全局+项目同时扫描） | ✅ 已修正（统一为同时扫描两侧） |
| dry-run 验收是否要求前序真实阶段 | ✅ 已修正（明确不要求） |