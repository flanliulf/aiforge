...
|----------|------|------------|------|----------|----------|
| CR-PROCESS-01 | 规则数量验收口径必须绑定已批准基线与本 Story 明确新增范围 | 7-3 | 7/12 | rules-summary | 已写入规则总结 |
...
|----------|----------|------|----------|--------------|
| 规则数量验收口径必须绑定已批准基线与本 Story 明确新增范围 | 通过 | 7/12 | rules-summary | 按流程默认推荐记录 |
...
- **量化评分**:
  ...
  | 维度 | 分数 | 理由 |
  |------|------|------|
  | 复现频次 | 1 | 单 Story 多轮出现，且由上游 Story 基线变化触发，有复现迹象。 |
...
- **全局文档建议**:
  - 不建议升格全局文档。该规则是 CR 流程中的数字口径处理经验，适合先记录在 CR 规则总结中，后续若多 Story 复现再升格。
- **本次落地**:
