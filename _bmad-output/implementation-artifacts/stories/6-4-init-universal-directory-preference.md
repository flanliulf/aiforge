...
- [ ] Task 1: 在 `aiforge init` 交互流程中添加通用目录偏好询问 (AC: #1, #2)
  - [ ] 1.1 在 `src/commands/init.ts` 的 `runInit()` 中，在连接验证成功后、配置保存之前，添加通用目录偏好询问步骤
  - [ ] 1.2 使用 `confirm()` 交互组件（已从 `@inquirer/prompts` 导入）：`const universalDirs = await confirm({ message: msg('init.universalDirsPrompt'), default: true })`
  - [ ] 1.3 询问位置：在认证方式选择 + 连接验证成功之后、`const config: AiforgeConfig = { ... }` 配置对象构建之前
  - [ ] 1.4 连接验证失败时不询问（因为 `connectionOk === false` 会直接 `return`，自然跳过后续步骤）
  - [ ] 1.5 已有配置修改场景：`existingConfig?.universalDirs` 作为 `confirm()` 的 `default` 值（若存在），否则默认 `true`
  - [ ] 1.6 注意：`confirm()` 已在 `init.ts` 第 18 行导入（用于"是否修改当前配置"询问），无需新增 import

- [ ] Task 2: 将 `universalDirs` 写入配置对象 (AC: #2)
  - [ ] 2.1 在 `runInit()` 的配置对象构建中追加 `universalDirs` 字段：
    ```typescript
    ...
    ```
  - [ ] 2.2 `saveConfig()` 已有逻辑直接 `JSON.stringify(config)` 序列化，`universalDirs` 字段自动持久化，无需修改 `services/config.ts`

- [ ] Task 3: 扩展 `AiforgeConfig` 类型 (AC: #5)
  - [ ] 3.1 **注意**：Story 6-3 已在 `src/core/types.ts` 的 `AiforgeConfig` 接口中添加 `universalDirs?: boolean`。如果 Story 6-3 先于 6-4 实现，此 Task 可跳过
  - [ ] 3.2 如果 Story 6-4 先于 6-3 实现：在 `AiforgeConfig` 接口中添加 `universalDirs?: boolean`（可选字段，缺省视为 `true`）
  - [ ] 3.3 `isValidConfigStructure()` 无需修改：该函数只校验 `auth` 字段是否为 object，不校验其他可选字段

- [ ] Task 4: 已有配置摘要展示 universalDirs 状态 (AC: #1)
  - [ ] 4.1 在 `runInit()` 的"显示当前配置摘要"部分（`console.log` 输出区域），追加一行展示 universalDirs 当前状态
  - [ ] 4.2 实现：`console.log(\`${msg('init.universalLabel')}${existingConfig.universalDirs !== false ? msg('init.enabled') : msg('init.disabled')}\`)`
  - [ ] 4.3 位置：在 `console.log(\`${msg('init.authLabel')}...\`)` 之后

- [ ] Task 5: 添加 i18n 消息键 (AC: #1)
  - [ ] 5.1 在 `src/core/messages.ts` 的 `MessageSet.init` 分组中添加：
...
- [ ] Task 6: 编写单元测试 (AC: #1-5)
  - [ ] 6.1 扩展 `tests/commands/init.test.ts`，在现有测试套件中新增 `describe('通用目录偏好 (AC Story 6-4)', () => { ... })` 块
  - [ ] 6.2 测试用例：
  ...
  - [ ] 6.3 测试 mock 模式：复用现有 `mocks.confirm` mock（在 `vi.hoisted` 中已定义），注意区分"是否修改配置" confirm 和 "universalDirs" confirm 的调用顺序
  - [ ] 6.4 `confirm` 调用顺序说明：
...
- [ ] Task 7: 跨 Story 验证 AC #3-#5 (AC: #3, #4, #5)
  - [ ] 7.1 确认 Story 6-3 已实现并通过以下测试：
  ...
  - [ ] 7.2 如果 Story 6-3 测试尚未覆盖上述场景，在本 Story 完成后通知 Story 6-3 补充对应测试（本 Story 不直接修改 Story 6-3 文件）
  - [ ] 7.3 **注意**：本 Story 自身测试（`tests/commands/init.test.ts`）只验证 config.json 写入正确，不验证运行时安装行为

- [ ] Task 8: 质量门禁验证 (AC: #1-5)
  - [ ] 8.1 `npm test` — 全绿
