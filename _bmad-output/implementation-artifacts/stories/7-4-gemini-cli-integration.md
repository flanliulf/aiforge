...
5. **Given** 执行 `npm test && npm run lint:src && npm run build`  
  **When** Story 7-4 实施完成  
  **Then** 全部通过，新增规则 +4 条，`BUILTIN_RULES` 总量为 33 条（29 + 4），零引擎代码改动

## Tasks / Subtasks

- [x] Task 1: 注册 gemini 工具定义 (AC: #1)
  - [x] 1.1 在 `src/data/tool-registry.ts` 追加：`{ id: 'gemini', name: 'Gemini CLI', detect: { global: ['~/.gemini'], project: ['.gemini'] } }`
  - [x] 1.2 注意：检测路径 `~/.gemini/` 与 antigravity 共享，但 antigravity 使用更深的 `~/.gemini/antigravity/` 子目录（Story 7-8）实现隔离
- [x] Task 2: 添加 gemini 安装规则 (AC: #2)
...
- [x] Task 3: 实现 Gemini 版本校验（NFR-I5 约束下的方案选型）(AC: #3)
  - [x] 3.1 **设计决策**：版本校验需调用外部进程 `gemini --version`，这是 NFR-I5 之外的"运行时探测"逻辑
  - [x] 3.2 优先方案：在 `src/data/install-rules.ts` 同文件追加 `TOOL_PRECONDITIONS: Record<string, { check: () => Promise<{ ok: boolean; reason?: string }>; affectedSourceDirs: string[] }>` 数据驱动结构（数据 + 闭包，仍属 data/ 范畴）
  - [x] 3.3 在 `src/services/` 新增 `version-check.ts`：导出 `checkGeminiVersion(): Promise<{ version: string | null; meetsRequirement: boolean }>`，使用 `child_process.execFile('gemini', ['--version'], { timeout: 2000 })`，捕获 ENOENT/超时/非零退出码，全部降级返回 `{ version: null, meetsRequirement: false }`
  - [x] 3.4 在 `src/stages/match-rules.ts` 末尾追加 precondition 检查（**Dev 评审**：此处属 Epic 7 例外允许的"数据驱动 + 工具特异性"扩展，需 PM 确认）：遍历 `TOOL_PRECONDITIONS`，若工具命中且 check 失败，从 items 中剔除 affectedSourceDirs 匹配的规则，并通过 reporter.warn 输出 `msg('precondition.geminiVersion')`
  - [x] 3.5 i18n 键新增：
  ...
  - [x] 3.6 **替代保守方案**：若 PM 否决 Task 3.3-3.4，则版本校验完全不实现，仅在 README/migration doc 中文档化"Gemini Skills 需要 v0.26.0+"，AC #3 降级为文档要求；规则数 +4 不变
- [x] Task 4: 编写单元测试 (AC: #1-5)
  - [x] 4.1 扩展 `tests/data/install-rules.test.ts`：`BUILTIN_RULES.length === 33`，包含 4 条 gemini 规则
  - [x] 4.2 扩展 `tests/stages/detect-tools.test.ts`：gemini 检测用例
  - [x] 4.3 若采纳 Task 3 方案：新增 `tests/services/version-check.test.ts`：
  ...
  - [x] 4.4 若采纳 Task 3 方案：扩展 `tests/stages/match-rules.test.ts`：
...
- 2026-05-18: `npm test -- tests/services/version-check.test.ts tests/data/install-rules.test.ts tests/stages/detect-tools.test.ts tests/stages/match-rules.test.ts` → 122/122 passed
- 2026-05-18: `npm test && npm run lint:src && npm run build` → 全量测试、lint:src、build 全部通过

### Completion Notes List

- 注册 Gemini 工具定义，检测标志为 `~/.gemini` 与 `.gemini`。
- 在 `src/data/install-rules.ts` 新增 4 条 Gemini 规则：全局 skills/instructions、项目 skills/instructions。
- 采用 Story 7-4 推荐方案实现 `TOOL_PRECONDITIONS`，将 Gemini 版本校验保持为数据驱动前置条件，而非在匹配引擎中为单工具硬编码分支。
- 新增 `src/services/version-check.ts`，用 `execFile('gemini', ['--version'])` 探测版本；对 ENOENT、超时、非零退出码统一降级为 `{ version: null, meetsRequirement: false }`。
- 在 `match-rules` 阶段应用 precondition：版本不足或未找到命令时，仅剔除 Gemini `skills` 规则，保留 `instructions` 规则，并通过 Reporter 输出明确升级/安装提示。
- 为中英文字典补充 `precondition.geminiVersion` 与 `precondition.geminiNotFound`，并补齐对应测试。
- 已完成用户要求的质量门禁：`npm test`、`npm run lint:src`、`npm run build` 全部通过。
- 规则总量口径按已批准基线同步为 33（Story 7-2 为 24，Story 7-3 为 29，本 Story 新增 4 条 Gemini 规则）；未为迎合旧数字额外添加超范围规则。
...
- tests/stages/detect-tools.test.ts
- tests/stages/match-rules.test.ts
- tests/services/version-check.test.ts
- _bmad-output/implementation-artifacts/stories/7-4-gemini-cli-integration.md
- _bmad-output/implementation-artifacts/sprint-status.yaml

### Change Log

- 2026-05-18: 完成 Gemini CLI 检测、安装规则与版本前置校验接入，补齐相关单元测试并通过质量门禁；Story 状态更新为 review。
