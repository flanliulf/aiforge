...
- [x] Task 1: 注册 trae 工具定义 (AC: #1)
  - [x] 1.1 在 `src/data/tool-registry.ts` 追加：`{ id: 'trae', name: 'Trae (ByteDance)', detect: { global: ['~/.trae'], project: ['.trae'] } }`
- [x] Task 2: 添加 trae 安装规则（不含 skills）(AC: #2)
  ...
  - [x] 2.2 共 2 条；与 epic AC #4 一致
  - [x] 2.3 **明确不添加** trae:skills 规则（Trae Skills 通过 UI 管理）
- [x] Task 3: 实现 Skills 不支持的提示 (AC: #3)
  - [x] 3.1 **设计决策**：当用户对 trae 工具执行安装且知识仓库有 `skills/` 目录时，需要明确告知"Trae Skills 不支持文件安装"
  - [x] 3.2 复用 Story 7-4 引入的 `TOOL_PRECONDITIONS` 数据结构（如已采纳）：
    ```typescript
  ...
  - [x] 3.3 **替代实现**：如果 7-4 的 TOOL_PRECONDITIONS 方案未采纳，则使用更轻量的方式：
...
- [x] Task 4: 编写单元测试 (AC: #1-4)
  ...
  - [x] 4.2 扩展 `tests/stages/detect-tools.test.ts`：mock `~/.trae/` → 检测命中
  - [x] 4.3 扩展 `tests/stages/match-rules.test.ts`：
...
- 2026-05-19: `npm test -- tests/data/tool-registry.test.ts tests/data/install-rules.test.ts tests/data/messages.test.ts tests/stages/detect-tools.test.ts tests/stages/match-rules.test.ts` → 首轮失败用于锁定缺失的 trae 定义/规则/消息/notice/info；修复后 208/208 通过
- 2026-05-19: `npm test` → 947/947 通过
- 2026-05-19: `npm run lint:src` → 通过，Prettier 检查通过
- 2026-05-19: `npm run build` → tsup ESM + DTS 构建通过

### Completion Notes List

- 新增 Trae 工具定义，自动检测路径为 `~/.trae` 和 `.trae`。
- 在 `src/data/install-rules.ts` 新增 2 条 trae 项目级规则：`rules -> .trae/rules/` 与 `instructions/AGENTS.md -> ./`。
- 未新增任何 trae skills 安装规则，保持 Story 7-9 对能力边界的要求。
- 采用 Story 允许的替代实现：新增 `TOOL_UNSUPPORTED_NOTICES`，在 `match-rules` 阶段检测到知识仓库含 `skills/` 且工具命中 trae 时，通过 `reporter.info()` 输出 `unsupported.traeSkills` 说明。
- 为 Reporter 增加 `info()`，确保该提示不被当作 warn/failure 呈现。
- 补充了 trae 的工具注册、规则矩阵、消息键、检测逻辑与 notice 行为测试，并通过全量质量门禁。
...
- _bmad-output/implementation-artifacts/code-reviews/7-9-code-review/EXPERIMENTS.md
- _bmad-output/implementation-artifacts/code-reviews/7-9-code-review/EXPERIMENT_NOTES.md

### Change Log

- 2026-05-19: 完成 Story 7-9 Trae 部分接入；新增 trae 工具定义、2 条项目级安装规则、skills 不支持 info 提示与对应测试，质量门禁全部通过。
