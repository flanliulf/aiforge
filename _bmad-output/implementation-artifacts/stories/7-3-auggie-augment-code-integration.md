...
- [ ] Task 1: 注册 auggie 工具定义 (AC: #1)
  - [ ] 1.1 在 `src/data/tool-registry.ts` 追加：`{ id: 'auggie', name: 'Auggie (Augment Code)', detect: { global: ['~/.augment'], project: ['.augment'] } }`
- [ ] Task 2: 添加 auggie 安装规则 (AC: #2, #3, #4)
  - [ ] 2.1 在 `src/data/install-rules.ts` 追加规则：
...
- [ ] Task 3: 实现 instructions 文件名过滤（AGENTS.md 跨工具标准）(AC: #4)
  - [ ] 3.1 **设计决策**：FR-058 要求 instructions 目录可同时存在 `CLAUDE.md` 与 `AGENTS.md`，按目标工具的约定分发（Claude 分发 `CLAUDE.md`，其他工具分发 `AGENTS.md`）
  - [ ] 3.2 现有 `Files` 类型的 `scanSourceFiles()` 会扫描 instructions 下所有文件并复制——会导致 auggie 既收到 `AGENTS.md` 又收到 `CLAUDE.md`
  - [ ] 3.3 解决方案（**优先方案 A**）：扩展 `InstallRule` 类型新增可选 `fileFilter?: string[]` 字段（白名单）；在 `scanSourceFiles()` 末尾追加白名单过滤；auggie instructions 规则设置 `fileFilter: ['AGENTS.md']`，claude instructions 规则（Story 7-1 添加的）设置 `fileFilter: ['CLAUDE.md']`
  - [ ] 3.4 **重要**：方案 A 涉及 `src/core/types.ts` 的 `InstallRule` 接口扩展 + `src/stages/match-rules.ts` 的 scanSourceFiles 逻辑扩展。这突破了 Epic 7 NFR-I5 "零引擎改动"边界。**Dev 阶段开工前必须发起对该方案的评审**：
  ...
  - [ ] 3.5 **当前 story 默认采用方案 A**，并将 fileFilter 字段加入 7-1 Story 7-3 7-7 7-9 的 instructions 规则。Dev 评审若决定改方案 C，则将 instructions 规则全部移除并在 7-10 重新设计
- [ ] Task 4: 编写单元测试 (AC: #1-5)
  - [ ] 4.1 扩展 `tests/data/install-rules.test.ts`：
    - `BUILTIN_RULES.length === 30`（前提：Task 3.4 选择方案 A 且 7-1/7-2/7-3 累积一致）
    - 包含 5 条 `tool === 'auggie'` 规则
    - auggie instructions 规则的 `fileFilter` 字段为 `['AGENTS.md']`（如方案 A 采纳）
  - [ ] 4.2 扩展 `tests/stages/detect-tools.test.ts`：mock `~/.augment/` → 检测命中
  - [ ] 4.3 若方案 A 采纳：扩展 `tests/stages/match-rules.test.ts`：
