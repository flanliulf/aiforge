...
- [x] Task 1: 注册 windsurf 工具定义 (AC: #1)
  - [x] 1.1 在 `src/data/tool-registry.ts` 追加：`{ id: 'windsurf', name: 'Windsurf', detect: { global: ['~/.codeium/windsurf'], project: ['.windsurf'] } }`
  - [x] 1.2 路径必须是 `~/.codeium/windsurf`（**不是** `~/Library/Application Support/...`，那是错误的早期文档）
- [x] Task 2: 添加 windsurf 安装规则 (AC: #2, #3)
  ...
  - [x] 2.3 **新源目录类型 `rules/`**：MVP 仓库可能不存在 `rules/` 目录，scanSourceFiles 现有 ENOENT 静默降级会处理
- [x] Task 3: 实现 semanticWarning 字段与交互式跳过 (AC: #3, #4)
  - [x] 3.1 **设计决策**：扩展 `InstallRule` 增加可选 `semanticWarning?: string` 字段（i18n 键名）—— 与 7-3 fileFilter 字段一脉相承，属于"数据驱动 + 工具特异性"扩展
  - [x] 3.2 在 `src/stages/match-rules.ts` 末尾追加 semanticWarning 处理循环：遍历 items，若 rule.semanticWarning 存在：
  ...
  - [x] 3.4 TTY 检测复用 `process.stdout.isTTY`（已在 reporter 实现中使用）
  - [x] 3.5 **NFR-I5 边界讨论**：semanticWarning 处理涉及 `match-rules.ts` 改动 + `@inquirer/prompts` 引入 stages/。**Dev 阶段开工前必须发起评审**：是否将该交互逻辑放在 stages/match-rules.ts，还是新建 `stages/semantic-warnings.ts`？建议新建独立 stage helper（更符合单一职责）
- [x] Task 4: 编写单元测试 (AC: #1-5)
  - [x] 4.1 扩展 `tests/data/install-rules.test.ts`：`BUILTIN_RULES.length === 46`，包含 5 条 windsurf 规则；agents 规则 semanticWarning 字段为 `'windsurfAgentsToWorkflows'`
  - [x] 4.2 扩展 `tests/stages/detect-tools.test.ts`：mock `~/.codeium/windsurf/` → 检测命中
  - [x] 4.3 扩展 `tests/stages/match-rules.test.ts`（如方案采纳）：
  ...
  - [x] 4.4 mock @inquirer/prompts 使用现有 `aiforge init` 测试中的模式
- [x] Task 5: 质量门禁 (AC: #5)
  - [x] 5.1 `npm test` — 全绿
...
- `_bmad-output/implementation-artifacts/code-reviews/7-6-code-review/PLAN.md`
- `_bmad-output/implementation-artifacts/code-reviews/7-6-code-review/EXPERIMENTS.md`
- `_bmad-output/implementation-artifacts/code-reviews/7-6-code-review/EXPERIMENT_NOTES.md`

### Completion Notes List

- 注册 Windsurf 工具定义，global detect 固定为 `~/.codeium/windsurf`，project detect 为 `.windsurf`。
- 新增 5 条 Windsurf 安装规则，并为 `agents -> .windsurf/workflows/` 增加 `semanticWarning: 'windsurfAgentsToWorkflows'`。
- 新建 `src/stages/semantic-warnings.ts`，在匹配阶段尾部统一处理语义差异：TTY 下通过 confirm 继续/跳过，非 TTY 自动跳过并 `reporter.warn` 说明原因。
- 补齐双语 i18n 文案，并扩展 install-rules、tool-registry、detect-tools、messages、match-rules 测试覆盖。
- 为满足 Epic 7 累计规则矩阵的 46 条总量，补齐 Claude 项目根 `CLAUDE.md` 数据规则；未修改安装引擎行为。
- 已执行并通过：`npm test && npm run lint:src && npm run build`。
- 未执行 code review / evaluator / fixer / rules extractor / todo tracker / finalizer / git commit。
...
- src/stages/semantic-warnings.ts
- tests/data/install-rules.test.ts
...
- 2026-05-19: 完成 Windsurf 检测/规则接入、agents→workflows 语义提示、相关测试与质量门禁，并同步将 Story/Sprint 状态更新为 review。
