...
- [x] Task 1: 扩展 `ParsedArgs` 和 CLI 参数解析 (AC: #1)
  - [x] 1.1 在 `src/core/types.ts` 的 `ParsedArgs` 接口中添加 `filter?: string` 字段（可选，值为用户指定的 filter pattern）
  - [x] 1.2 在 `src/index.ts` 的 commander 定义中添加 `--filter <pattern>` 选项：`.option('--filter <pattern>', 'filter subdirectories by glob pattern')`
  - [x] 1.3 在 `src/pipeline.ts` 的 `mapOptsToArgs()` 中映射 `filter` 字段：`filter: opts['filter'] as string | undefined`
  - [x] 1.4 CLI help 文案使用英文硬编码

- [x] Task 2: 实现 filter pattern 解析逻辑 (AC: #1, #2, #3)
  - [x] 2.1 创建 `src/stages/filter-utils.ts`，导出 `parseFilterPattern(pattern: string): { dirPrefix?: string; glob: string }`
  - [x] 2.2 解析规则：
    - `skills/git*` → `{ dirPrefix: 'skills', glob: 'git*' }`（带顶层目录前缀）
    - `git*` → `{ dirPrefix: undefined, glob: 'git*' }`（无前缀，匹配所有顶层目录）
    - 分割点为第一个 `/`：`/` 左侧为 `dirPrefix`，右侧为 `glob`
  - [x] 2.3 导出 `matchesGlob(name: string, glob: string): boolean` — 简单 glob 匹配（支持 `*` 通配符和 `?` 单字符匹配，MVP 不需要 `**` 递归）
  - [x] 2.4 实现注意：不引入外部 glob 库（如 minimatch），使用简单正则转换即可覆盖 `*` 和 `?` 场景（MVP 够用）

- [x] Task 3: 在 Match 阶段集成 filter 逻辑 (AC: #1, #2, #3, #5)
  - [x] 3.1 在 `src/stages/match-rules.ts` 的 `matchRules()` 主循环中集成 filter 逻辑：过滤位置统一为 `scanSourceFiles` 返回后、`items.push` 之前（**不修改 `scanSourceFiles()` 函数本身**，保持其为纯扫描职责）
  - [x] 3.2 过滤逻辑位置：在 `scanSourceFiles` 返回后、`items.push` 之前，对 `sourceFiles` 执行二次过滤
  - [x] 3.3 具体实现：在 `matchRules()` 主循环内，`scanSourceFiles` 返回后：
    ```
    如果 args.filter 非空：
  ...
  - [x] 3.4 `--dirs` 与 `--filter` 联合语义（AC #2）：`--dirs` 先过滤规则级别（已有逻辑），`--filter` 再过滤文件级别。两者正交、叠加，无需特殊交互逻辑
  - [x] 3.5 注意 `--filter` 对三种 InstallType 的影响：
  ...
  - [x] 3.6 dry-run 一致性（AC #5）：filter 在 Match 阶段执行，Match 输出同时供 Install 和 Report 消费。filter 后必须剔除空 item（Task 3.3 守卫），以保证 dry-run 预览中不包含未匹配规则的 `emptySourceDir` 输出行

- [x] Task 4: 实现零匹配交互式询问 (AC: #4)
  - [x] 4.1 在 `src/stages/match-rules.ts` 的 `matchRules()` 中，全部规则处理完毕后检测：如果 `args.filter` 非空，且最终 `items` 中所有条目的 `sourceFiles` 均为空，触发零匹配处理
  - [x] 4.2 零匹配处理逻辑：
  ...
  - [x] 4.3 用户选择后：用局部变量 `resolvedFilter` 保存用户选择的**限定名称**（如 `skills/git-commit`，必须保留顶层目录前缀 `dirPrefix/`），重新执行 `scanSourceFiles` + filter 逻辑；**禁止改写 `args.filter`**（`ParsedArgs` 只读原则，改写 args 会丢失原始 `dirPrefix` 作用域，导致重新匹配时跨目录扩散）
  - [x] 4.4 用户取消：抛出独立 sentinel class `FilterCancelledSignal`（**不继承 `AiforgeError`**，也不继承 `Error` 的错误语义——取消是正常流而非错误），管道编排器用 `instanceof FilterCancelledSignal` 识别后以 exit code 0 正常退出——**禁止**返回 `{ items: [] }` 空计划（与"合法空安装计划"结构不可区分）；**禁止**使用 `AiforgeError`（`Severity = 'fatal' | 'partial'` 类型不含 `'info'`，编译直接失败）
  - [x] 4.5 TTY 检测：使用 `process.stdin.isTTY`（与 `commands/init.ts` 模式一致）
  - [x] 4.6 注意：`@inquirer/prompts` 是已有依赖（`commands/init.ts` 已使用），不引入新依赖

- [x] Task 5: 添加 i18n 消息键 (AC: #1, #4)
  - [x] 5.1 在 `src/core/messages.ts` 的 `MessageSet` 接口中添加 `filter` 分组
  - [x] 5.2 添加中英文消息键：
...
- [x] Task 6: 编写单元测试 (AC: #1-5)
  - [x] 6.1 创建 `tests/stages/filter-utils.test.ts`
    - 测试 `parseFilterPattern()`：带前缀、不带前缀、边界输入
    - 测试 `matchesGlob()`：`*` 通配符、`?` 单字符、精确匹配、大小写
  - [x] 6.2 扩展 `tests/stages/match-rules.test.ts`
  ...
  - [x] 6.3 扩展 `tests/pipeline.test.ts`：`--filter` + `--dry-run` 预览结果只包含匹配条目
  - [x] 6.4 新增错误码 `FILTER_NO_MATCH` 从真实创建模块入口触发的负向测试
  - [x] 6.5 扩展 `tests/pipeline.test.ts`：`FilterCancelledSignal` 编排器捕获行为
...
- [x] Task 7: 质量门禁验证 (AC: #1-5)
  - [x] 7.1 `npm test` — 全绿（775 tests / 32 files）
...
- Task 2-4 中 Filter 逻辑集成在 `src/stages/match-rules.ts`，不修改 `scanSourceFiles()` 函数本身
- `FilterCancelledSignal` 定义在 `src/stages/filter-utils.ts`（非 errors.ts），pipeline.ts 导入
- 空条目守卫（empty-item guard）是通用行为，适用于有/无 filter 两种场景；8 个既有测试相应更新
- TTY 零匹配：`select()` 的候选值使用限定名（含 dirPrefix 前缀），确保重新匹配时作用域不丢失
- Task 6.4（cli-args.test.ts）补充了 `--filter` 字段映射验证；`mapOptsToArgs` 直接测试
- 全量 775 个测试通过，lint 和 build 均无错误

### File List

**新增文件：**
- `src/stages/filter-utils.ts`
