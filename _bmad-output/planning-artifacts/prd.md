---
stepsCompleted: ['step-01-init', 'step-02-discovery', 'step-03-success', 'step-04-journeys', 'step-05-domain', 'step-06-innovation', 'step-07-project-type', 'step-08-scoping', 'step-09-functional', 'step-10-nonfunctional', 'step-11-polish', 'step-12-complete', 'step-e-01-discovery', 'step-e-02-review', 'step-e-03-edit-epic-7-2026-04-21']
classification:
...
editHistory:
  - date: 2026-04-21
    ...
    changes: 'Epic 7 工具覆盖扩展：新增 12 条 FR (FR-047~FR-058) 与 3 条 NFR (P6/I5/C7)，修订工具清单章节、成功标准追踪矩阵、Growth Features；主版本号跳变以标示 VS Code 归并 Breaking Change'
---

# Product Requirements Document - ai-forge

**Author:** chunxiao
**Date:** 2026-03-11（初版）/ 2026-04-21（Epic 7 扩展）
**Version:** v2.0.0-draft（Epic 7 扩展草案；MVP 基线 v1.0.0）
**Status:** Draft（Epic 7 扩展中）
...
**MVP 聚焦（v1.0）：** 公司内部开发者和平台维护者的核心场景，支持 Copilot + Claude + Cursor + VS Code 四个工具，同时按社区级通用工具的标准进行架构设计。

**Epic 7 扩展（v2.0，Breaking Change）：** 从 4 工具 MVP 演进为覆盖 11 个主流 AI IDE / 代码助手的标准化安装器：新增 8 个工具接入（Auggie、Codex CLI、Gemini CLI、OpenCode、Windsurf、Kiro、Antigravity、Trae），归并 `vscode` 独立工具（原 1 条 mcp-tools 规则迁移到 Copilot 语境下的 `.vscode/mcp.json`），明确不支持已停服的 iFlow CLI。规则矩阵从 16 条扩展至约 54 条，通过数据驱动架构（`src/data/`）完成，不改动引擎代码。该版本跳变至 v2.0.0 以明确信号此为向后不兼容的里程碑。
...
| **零结果诊断** | 当安装场景中「未产生任何可处理项」（`resultItems.length === 0`）时自动触发的诊断输出，包含扫描了哪些目录、匹配了哪些规则、如何修复（分 `noInstallSources` 与 `emptyDirectories` 两类）。**不包括**「全部跳过」场景（`resultItems.length > 0` 但无 new/updated）——后者属成功路径，输出「没有新增/更新文件，全部已是最新或被跳过」成功摘要。 |
...
| **InstallType 降级策略** | 当目标工具的配置需写入结构化配置文件（如 TOML/JSON）而非独立文件时，ai-forge 采用"复制模板文件 + Reporter 输出手动合并提示"的降级路径，避免引入结构化配置编辑能力 |
| **XDG_CONFIG_HOME 约定** | Linux/macOS 用户全局配置路径规范（默认 `~/.config/<tool>/`），OpenCode 等工具依此约定；ai-forge 的 `detect.global` 和 `targetDir` 遵循此规范 |
| **AGENTS.md 标准** | 11 个候选 AI 工具中 10 个兼容的跨工具指令文件约定（Claude Code 坚持 `CLAUDE.md`）；知识仓库 `instructions/` 目录允许 `AGENTS.md` 与 `CLAUDE.md` 共存并按工具分发 |
| **Breaking Change** | 向后不兼容的变更；在 ai-forge 语境特指 Epic 7 对 `vscode` 独立工具的归并——既有用户的 `~/.vscode/` 安装将被弃用，需通过 migration note + CHANGELOG + 主版本号跳变（v1.x→v2.0）明示 |
| **Antigravity 子目录隔离** | Antigravity 与 Gemini CLI 共享 `~/.gemini/` 根目录时，用 `~/.gemini/antigravity/` 独占子目录作为 `detect.global`，避免工具识别冲突 |
...
| 结果 | 基线（当前） | MVP 目标（v1.0） | Epic 7 目标（v2.0） |
|------|------------|------------------|---------------------|
| AI 工具配置覆盖 | 仅 Copilot（Shell 脚本） | Copilot + Claude + Cursor + VS Code（4 工具） | 11 工具生态：copilot/claude/cursor/codex/auggie/gemini/opencode/windsurf/kiro/antigravity/trae（剔除独立 `vscode`、剔除停服的 iFlow） |
| 安装方式 | 2 个独立脚本 | 1 条命令 | 1 条命令（保持不变） |
| 跨平台 | 仅 macOS（WSL for Windows） | macOS + Linux（Windows M2） | macOS + Linux（Windows 仍 M2 级） |
| 新工具接入成本 | 编写新脚本 + 多平台测试 | 添加配置规则 | 添加配置规则（Epic 7 验证数据驱动：8 个工具仅改 `src/data/`） |
| 规则矩阵规模 | N/A | 16 条（4 工具 × 源目录组合） | ~54 条（+38/-1/5 微调，见 §安装规则映射表） |
...
| Epic 7 工具扩展 | 旅程 4 | FR-047~FR-058 | NFR-P6, NFR-I5, NFR-C7 |
...
- FR-032: 系统在「未产生任何可处理项」（`resultItems.length === 0`）时触发零结果诊断模式；「全部跳过」场景视为成功路径，输出「没有新增/更新文件，全部已是最新或被跳过」摘要而不触发诊断
...
> **Epic 7 扩展说明（v2.0）：** 本表为 MVP 基线（16 条，4 工具）。Epic 7 完成后规则矩阵将扩展至约 54 条（+38 新规则 / -1 VS Code 删除 / 5 条既有微调），覆盖 11 工具生态。完整矩阵详见 `docs/install-rules-matrix.md` 与 `docs/install-rules-matrix.zh.md`（由 Epic 7 Story 7-10 收尾交付）；按工具/范围/源目录/安装类型/目标路径五维一致建模，不改变 `InstallType` 引擎能力（Files/Directories/Flatten 够用，MCP 特殊场景采用 FR-052 降级策略）。
...
| NFR-P6 | Epic 7 扩展后 11 工具检测总耗时 | < 1000 毫秒（≤ NFR-P5 基线 2 倍） | 11 工具全扫描计时（并行检测方式） |
...
| NFR-C5 | 路径处理支持跨平台分隔符，不硬编码 `/` 或 `\` | 代码审查 |
| NFR-C6 | Home 目录解析失败时输出明确错误并退出，不静默继续 | 模拟 HOME 未设置 |
| NFR-C7 | 从 MVP（v1.0）升级到 Epic 7（v2.0）的现有用户需平滑迁移：`aiforge` 运行不得破坏性覆盖或删除 `~/.vscode/` 中 MVP 已安装的 MCP 配置；归并语义变更通过 migration note + CHANGELOG + `--dry-run` 提示显式化 | 模拟 MVP 用户环境升级到 v2.0 后运行全流程 |
...
| NFR-I5 | 新增 AI 工具或安装规则无需修改引擎代码，仅通过工具注册表与规则配置变更完成（Epic 7 验证：8 个新工具接入不改引擎实现层） | 代码审查 + 集成测试覆盖率统计 |
...
- `aiforge list` 工具路径列表
- **Epic 7：AI IDE 工具覆盖扩展** — 从 4 工具 MVP 扩展至 11 工具生态（详见下文专项章节）
...
**目标：** 将 ai-forge 从"MVP 阶段 4 工具支持"演进为"AI 编码助手生态的标准化安装器"。

**范围（11 工具最终清单）：**
`copilot · claude · cursor`（MVP 既有，微调补齐规则）+ `codex · auggie · gemini · opencode · windsurf · kiro · antigravity · trae`（新增 8 工具）。剔除 `vscode`（归并到 Copilot）与 `iflow`（已停服 2026-04-17）。

**规模与节奏：** 10 个 Story（Story 7-1 ~ 7-10）/ 约 +38 条规则 / -1 条删除 / 5 条既有微调 / 2-3 Sprint 周期。

**前置条件：** MVP（Epic 1-5）已交付 + Epic 6（通用目录支持、精细化安装控制）已完成。

**Breaking Change 处置：**
- 主版本号跳变 v1.x → v2.0.0
- Story 7-1 作为首发，专项处理 VS Code 归并：删除 `vscode` TOOL_DEFINITION、迁移 1 条原规则到 Copilot 语境（`.vscode/mcp.json` 项目级 MCP）、输出 migration note + CHANGELOG
- 检测到既有 `~/.vscode/` 用户时给出兼容提示，不破坏性覆盖（见 FR-050、NFR-C7）

**架构影响：** 不新增 InstallType（Files/Directories/Flatten 覆盖 95% 场景）；MCP 配置写入结构化文件（Codex TOML / OpenCode JSON）的 5% 场景采用 FR-052 降级策略（模板 + 手动合并），保持引擎零入侵。

**关键 Story 映射：**

| Story | 主题 | 关联 FR/NFR |
|-------|------|-------------|
| 7-1 | VS Code 归并 + Copilot/Claude/Cursor 规则补齐 | FR-050, NFR-C7 |
| 7-2 | Codex CLI 接入（MCP 降级） | FR-047, FR-052 |
| 7-3 | Auggie (Augment) 接入 | FR-047, FR-057 |
| 7-4 | Gemini CLI 接入（v0.26.0+） | FR-047, FR-055, FR-057 |
| 7-5 | OpenCode 接入（XDG 路径） | FR-047, FR-048, FR-052 |
| 7-6 | Windsurf 接入（语义差异） | FR-047, FR-053 |
| 7-7 | Kiro (AWS) 接入 | FR-047, FR-057 |
| 7-8 | Antigravity 接入（子目录隔离） | FR-047, FR-049 |
| 7-9 | Trae 部分接入（不含 Skills） | FR-047, FR-054 |
| 7-10 | 文档 + 矩阵表 + 回归测试收尾 | FR-047, NFR-I5 |

**排除项：** iFlow CLI（已停服 FR-051）；不新增 InstallType；不实现 TOML/JSON 结构化编辑。
...
| **Epic 7：VS Code 归并导致 MVP 用户迁移失败** | 低 | 高 | 主版本号 v1.x→v2.0 明示；Story 7-1 交付 migration note + CHANGELOG；FR-050 兼容检测；NFR-C7 平滑迁移验证 |
| **Epic 7：扩展工具路径约定变更（如 Windsurf `~/.codeium/windsurf/`、OpenCode `~/.config/opencode/`）** | 中 | 中 | data/ 驱动设计（NFR-I5），路径修正仅需 `src/data/` 单点调整；研究阶段已做多源交叉验证与路径修正记录 |
...
> 已原地更新上文词表与 FR-032，本块保留变更对照供审计追溯。

| 章节 / 行 | 变更前 | 变更后 | 依据 |
|----------|--------|--------|------|
| 词表·零结果诊断 | 「安装结果为 0 项时触发」（包含全部跳过） | 仅 `resultItems.length === 0` 触发；全部跳过视为成功路径 | 代码：src/stages/execute-install.ts / 测试：tests/integration/edge-cases.test.ts |
| FR-032 | 未区分「零项」与「全跳过」 | 明确「全部跳过」输出成功摘要而不触发诊断 | 代码：src/stages/execute-install.ts / 消息：src/core/messages.ts `executeInstall.skippedOnlySummary` |
