...
| **单文件失败不终止** | 某个文件安装失败时标记 failed，继续处理下一个 |
...
创建符号链接时，`src`（链接目标）可以是绝对路径或相对路径。需要决定采用哪种方式。

### 备选方案

| 方案 | 示例 |
|------|------|
| **A. 绝对路径（选定）** | `ln -s /Users/dev/aicoding-base/agents ~/.copilot/agents` |
| **B. 相对路径** | `ln -s ../../aicoding-base/agents ~/.copilot/agents` |
...
1. **全局安装的语义**：`~/.copilot/agents` → `~/aicoding-base/agents`，两个路径在不同的目录层级，相对路径需要多层 `../` 回退，容易出错
2. **可读性**：`ls -la ~/.copilot/` 看到的链接目标是绝对路径，一目了然
3. **不会因链接位置变化而断裂**：如果未来 AI 工具的配置目录发生变化，绝对路径的链接仍然指向正确的源文件
4. **项目级安装不涉及**：项目级安装不使用符号链接（用复制模式），所以不存在「项目可移动性」的担忧

### 影响

- `createSymlink()` 内部使用 `path.resolve()` 确保源路径为绝对路径
- 用户 Home 目录变更时链接可能断裂（极端场景，可通过重新安装修复）
...
## ADR-008: 全局排除列表而非 .aiforgeIgnore
...
| ADR-006 | 符号链接使用绝对路径 | 已采纳 |
| ADR-007 | 单命令入口 + 子命令结构 | 已采纳 |
| ADR-008 | 全局排除列表而非 .aiforgeIgnore | 已采纳 |
